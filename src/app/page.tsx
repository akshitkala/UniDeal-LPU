...
import { ArrowRight, ChevronRight, PackageCheck, AlertTriangle, CheckCircle2, Zap, ArrowUp } from 'lucide-react';
  ...
  return <div className="flex flex-col">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-6">
          <div className="grid lg:grid-cols-[45%_55%] gap-12 items-center">
            ...
            <div className="flex flex-col items-start text-left max-w-2xl mx-auto lg:mx-0">
              ...
              <p className="text-sm text-gray-700 leading-6 max-w-lg mb-8">List what you have. Find what you need. The safest way to trade with your peers.</p>
              
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                <Link href="/browse" className="h-10 lg:h-11 px-5 lg:px-6 bg-[#16a34a] hover:bg-green-700 text-white rounded-full font-semibold text-sm flex items-center justify-center transition-all">Browse listings</Link>
                <Link href="/post" className="h-10 lg:h-11 px-5 lg:px-6 border border-gray-200 bg-white text-gray-900 rounded-full font-semibold text-sm flex items-center justify-center transition-all hover:bg-gray-50">Start selling →</Link>
              </div>
            </div>
      ...
      <section className="bg-white py-10 lg:py-16">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-6">
          ...
          <div className="flex justify-between items-end mb-8">
            ...
            <Link href="/browse" className="text-sm font-semibold text-[#16a34a] hover:text-green-700 flex items-center gap-1 transition-colors">View all<ChevronRight className="w-4 h-4" />
            </Link>
          </div>

          {error && <div className="p-10 bg-red-50 text-red-600 rounded-3xl border border-red-100 flex items-center justify-center gap-3">
              <AlertTriangle className="w-6 h-6" /> {error}
            </div>}

          {loading ? <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-8">
              {[...Array(8)].map((_, i) => <div key={i} className="aspect-[4/5] bg-gray-50 rounded-2xl border border-gray-100 animate-pulse" />)}
            </div> : listings.length === 0 ? <div className="flex flex-col items-center justify-center py-24 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
              ...
              <Link href="/post" className="h-12 px-8 bg-[#16a34a] text-white rounded-full font-bold shadow-lg shadow-green-600/10 active:scale-95 transition-all">Post Item</Link>
            </div> : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-8">
              {listings.map((listing, index) => <ListingCard key={listing._id} listing={listing} priority={index < 4} />)}
            </div>}
        </div>
      </section>
      ...
      <section className="bg-white py-10 lg:py-16">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-6">
          <div className="text-center mb-10">
            ...
            <h2 className="text-base lg:text-lg font-semibold text-gray-900 leading-tight">A better way to trade on campus</h2>
            <p className="text-sm text-gray-500 mt-2 max-w-xl mx-auto">UniDeal is built to solve the mess of WhatsApp groups and fragmented campus deals.</p>
          </div>
          ...
          }].map((card, i) => <div key={i} className="bg-gray-50 rounded-xl p-6 border-l-2 border-[#16a34a] h-full flex flex-col">
      ...
      <section id="how-it-works" className="bg-gray-50 py-10 lg:py-16 border-y border-gray-100">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-6">
          ...
          }].map((step, i) => <div key={i} className="bg-white rounded-xl p-8 border border-gray-100 shadow-sm">
      ...
      <section className="bg-white py-10 lg:py-16">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-6">
          <div className="bg-[#16a34a] rounded-2xl p-10 lg:p-16 text-center text-white">
            <h2 className="text-2xl lg:text-3xl font-bold mb-4">Turn your clutter into cash</h2>
            <p className="text-sm opacity-90 max-w-xl mx-auto mb-8">The fastest way to sell on campus. Join thousands of students buying and selling locally.</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <Link href="/post" className="h-10 lg:h-11 px-6 bg-white text-[#16a34a] font-semibold text-sm rounded-full flex items-center justify-center transition-all hover:bg-gray-50">Post an item</Link>
              <Link href="/browse" className="h-10 lg:h-11 px-6 bg-green-700 text-white font-semibold text-sm rounded-full flex items-center justify-center transition-all hover:bg-green-800">Browse feed</Link>
            </div>
          </div>
        </div>
      </section>
