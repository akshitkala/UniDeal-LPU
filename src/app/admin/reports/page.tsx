...
export default function AdminReportsPage() {
  ...
  const fetchReports = async (status: string, cursor: string | null = null) => {
    ...
    try {
      ...
      if (cursor) {
        setReports(prev => {
          ...
          const newItems = data.reports.filter((r: any) => !existingIds.has(r._id))
          ...
          {nextCursor && (
            <div className="mt-8 flex justify-center">
                <button 
                  ...
                  className="h-10 px-8 bg-white border border-gray-200 rounded-full text-xs font-bold text-gray-600 hover:bg-gray-50 transition-all flex items-center gap-2 disabled:opacity-50"
                >
                  {loadingMore ? (
                    <>
