...
export function Navbar() {
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex h-8 bg-gray-900 text-white text-[10px] sm:text-xs items-center justify-center relative z-[60]">
        <p className="font-medium">Campus marketplace — buy and sell, simply.</p>
      </div>

      <nav className="sticky top-0 left-0 right-0 h-16 bg-white border-b border-gray-100 shadow-sm z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between gap-4">
          
          {/* Left: Logo */}
          <Link href="/" className="flex items-center gap-2 group shrink-0 min-w-[44px] min-h-[44px]">
            <div className="w-8 h-8 bg-[#16a34a] rounded-lg flex items-center justify-center transform group-hover:rotate-12 transition-transform shadow-sm">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            {/* Search Icon Button (Tablet/Mobile, or as extra on desktop) */}
            <button 
              onClick={() => setIsSearchOpen(!isSearchOpen)}
              className={cn(
                "lg:hidden w-10 h-10 flex items-center justify-center rounded-full text-gray-600 hover:bg-gray-100 transition-colors",
            ...
            {loading ? (
              ...
              <div className="flex items-center gap-3">
                <Link 
                  href="/post" 
                  className="hidden lg:flex bg-[#16a34a] hover:bg-green-700 text-white px-6 py-2 rounded-full font-semibold text-sm transition-all"
                >
                  Sell Now
                </Link>
                ...
                <div className="hidden lg:block relative">
                  ...
                  >
                    <Avatar 
                      ...
                      size="sm"
                    />
                    <ChevronDown className={cn("w-4 h-4 text-gray-500 transition-transform hidden sm:block", isProfileOpen && "rotate-180")} />
                  </button>
              ...
              <div className="hidden lg:flex items-center gap-2">
                <Link 
                  href={loginUrl} 
                  className="text-sm font-semibold text-gray-600 px-4 py-2 hover:text-gray-900 transition-all"
                >
                  Sign In
                </Link>
                <Link 
                  href="/post" 
                  className="bg-[#16a34a] hover:bg-green-700 text-white px-6 py-2 rounded-full font-semibold text-sm transition-all shadow-sm"
                >
                  Sell Now
                </Link>
              </div>
            ...
            <button 
              className="lg:hidden w-11 h-11 flex items-center justify-center text-gray-600 hover:bg-gray-50 rounded-full"
        ...
        {isSearchOpen && (
            <div className="absolute top-16 left-0 right-0 bg-white border-b border-gray-100 px-4 py-3 animate-slide-down z-40 lg:hidden">
                <div className="relative w-full max-w-2xl mx-auto">
                    ...
                    <input 
                        ...
                        onChange={(e) => setSearchQuery(e.target.value)}
                        onKeyDown={handleSearch}
                        className="w-full h-12 pl-11 pr-4 rounded-full bg-gray-100 border-none focus:ring-2 focus:ring-[#16a34a]/20 transition-all font-medium text-sm"
                    />
                </div>
            </div>
        )}
